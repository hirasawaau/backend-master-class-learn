...
func (q *Queries) FindAccount(ctx context.Context, id int64) (Account, error) {
	row := q.queryRow(ctx, q.findAccountStmt, findAccount, id)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
...
func (q *Queries) FindAccounts(ctx context.Context, arg FindAccountsParams) ([]Account, error) {
	rows, err := q.query(ctx, q.findAccountsStmt, findAccounts, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
